...
def get_real_price(pair_info):
try:
url = f”https://api.frankfurter.app/latest?from={pair_info[‘from’]}&to={pair_info[‘to’]}”
res = requests.get(url, timeout=5)
data = res.json()
return data[“rates”][pair_info[“to”]]
except:
return pair_info[“base”] + (random.random() - 0.5) * pair_info[“base”] * 0.001
...
return 100 - (100 / (1 + avg_gain / (avg_loss if avg_loss > 0 else 1e-9)))
...
r_up = rv > 52; r_dn = rv < 48; r_ob = rv > 78; r_os = rv < 22
...
hi9  = max(highs[n-8:n+1]); lo9  = min(lows[n-8:n+1])
hi26 = max(highs[max(0,n-25):n+1]); lo26 = min(lows[max(0,n-25):n+1])
tk = (hi9 + lo9) / 2; kj = (hi26 + lo26) / 2
...
send_telegram(
“🤖 *BINARY PRO MAX V4* เริ่มทำงานแล้ว!\n\n”
“📡 ราคา *Real-Time* จาก Frankfurter API\n”
...
price = get_real_price(info)
prev  = last_prices.get(pair, price)
last_prices[pair] = price
chg = ((price - prev) / prev * 100) if prev else 0
chg_str = f”+{chg:.4f}%” if chg >= 0 else f”{chg:.4f}%”
